  ...
  case "$OS_TYPE" in
    ...
    macos)
      if ! xcode-select -p >/dev/null 2>&1; then
        step "Installing Xcode Command Line Tools"; xcode-select --install || true
      fi ;;
...
ensure_latest_python() {
  if [[ ! -d "$HOME/.pyenv" ]]; then
    step "Install pyenv"; curl -fsSL https://github.com/pyenv/pyenv-installer/raw/master/bin/pyenv-installer | bash || warn "pyenv installer failed"
  else
    note "pyenv present"
  fi
  ...
  if command -v pyenv >/dev/null 2>&1; then
    ...
    [[ -z "$latest" ]] && { warn "Could not determine latest Python"; return 0; }
    if printf '%s\n' "$existing" | grep -qx "$latest"; then
      note "Latest Python $latest already installed"; pyenv global "$latest" || true; success "Python $latest active"; return 0
    fi
    step "Installing Python $latest (pyenv)"; install_python_build_deps
